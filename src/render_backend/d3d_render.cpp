...
static ID3D11Buffer* global_vertexBuffer_line;
...
typedef struct {
	...
	ID3D11Buffer* instancing_vertex_buffer_line;
	ID3D11InputLayout* glyph_inputLayout;
	ID3D11InputLayout* line_inputLayout;
	...
	UINT line_instance_buffer_stride;
	UINT quad_stride;
	UINT quad_numVerts;
...
static UINT backendRender_init(BackendRenderer *r, HWND hwnd) {
	...
	//NOTE: Create default vertex buffer shapes like Quad, Cube, Sphere
	{
		// Create Vertex Buffer
		{
		    ...
		    r->line_stride = 2 * sizeof(float);
		    r->line_numVerts = sizeof(vertexData) / r->line_stride;
		    ...
		    HRESULT hResult = d3d11Device->CreateBuffer(&vertexBufferDesc, &vertexSubresourceData, &global_vertexBuffer_line);
		    assert(SUCCEEDED(hResult));
		}
	...
	{ 
		...
		}
		{
			...
			D3D11_INPUT_ELEMENT_DESC inputElementDesc[] =
			{
				...
				{ "POSA_INSTANCE", 0, DXGI_FORMAT_R32G32B32_FLOAT, 1, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1 },
				{ "POSB_INSTANCE", 0, DXGI_FORMAT_R32G32B32_FLOAT, 1, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1 },
				{ "COLOR_INSTANCE", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_INSTANCE_DATA, 1 }, //NOTE: 1 at the end to say advance every instance, the reason this could be more than 1 is that the instance data might be for every 4 instances like each side of a face if each side represents the an instance, than if we wanted it to be the same color for all faces.  
			};

			HRESULT hResult = d3d11Device->CreateInputLayout(inputElementDesc, ARRAYSIZE(inputElementDesc), lineShader.vsBlob->GetBufferPointer(), lineShader.vsBlob->GetBufferSize(), &r->line_inputLayout);
			assert(SUCCEEDED(hResult));
		}
	...
	{ //NOTE: create the instancing_vertex_buffer for lines
	    D3D11_BUFFER_DESC vertexBufferDesc = {};
	    vertexBufferDesc.ByteWidth = (UINT)LINE_INSTANCE_DATA_TOTAL_SIZE_IN_BYTES;
	    vertexBufferDesc.Usage     = D3D11_USAGE_DYNAMIC;
	    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	    r->line_instance_buffer_stride = SIZE_OF_LINE_INSTANCE_IN_BYTES;

	    HRESULT hResult = d3d11Device->CreateBuffer(&vertexBufferDesc, 0, &r->instancing_vertex_buffer_line);
	    assert(SUCCEEDED(hResult));
	}
	...
	for(int i = 0; i < r->commandCount; ++i) {
		...
		switch(c->type) {
			...
			case RENDER_LINE: {
				...
				//NOTE: Update the vertex buffer
				D3D11_MAPPED_SUBRESOURCE mappedSubresource;
				d3d11DeviceContext->Map(backend_r->instancing_vertex_buffer_line, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
				u8* data_on_gpu = (u8*)(mappedSubresource.pData);
				
				memcpy(data_on_gpu, data, sizeInBytes);

				d3d11DeviceContext->Unmap(backend_r->instancing_vertex_buffer_line, 0);
				//////////
				
				d3d11DeviceContext->OMSetRenderTargets(1, &backend_r->default_d3d11FrameBufferView, nullptr);

				d3d11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
				d3d11DeviceContext->IASetInputLayout(backend_r->line_inputLayout);

				ID3D11Buffer *vertex_buffers[] = {global_vertexBuffer_line, backend_r->instancing_vertex_buffer_line};
				UINT buffer_strides[] = {backend_r->line_stride, backend_r->line_instance_buffer_stride};
				UINT offsets[] = {0, 0};

				d3d11DeviceContext->IASetVertexBuffers(0, 2, vertex_buffers, buffer_strides, offsets);

				d3d11DeviceContext->DrawInstanced(backend_r->line_numVerts, c->instanceCount, 0, 0);
